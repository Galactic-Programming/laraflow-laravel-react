/**
 * Sortable Cards Components
 *
 * This module provides two sets of drag-and-drop components:
 *
 * 1. **Kanban Components** - For multi-column board layouts (like Trello)
 *    - `Kanban` - Root wrapper with drag context
 *    - `KanbanBoard` - Container for columns
 *    - `KanbanColumn` - Individual column (draggable)
 *    - `KanbanColumnContent` - Content area within column
 *    - `KanbanColumnHandle` - Drag handle for column
 *    - `KanbanItem` - Individual item within a column
 *    - `KanbanItemHandle` - Drag handle for item
 *    - `KanbanOverlay` - Drag overlay for visual feedback
 *
 * 2. **Sortable Components** - For simple list reordering
 *    - `Sortable` - Root wrapper with drag context
 *    - `SortableItem` - Individual sortable item
 *    - `SortableItemHandle` - Drag handle for item
 *
 * @example
                ...
                {activeId ? (
                    <div className="z-50">
                        {React.Children.map(children, (child) => {
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            if (React.isValidElement(child) && (child.props as Record<string, any>).value === activeId) {
                                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                return React.cloneElement(child as React.ReactElement<Record<string, any>>, {
                                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                    ...(child.props as Record<string, any>),
                                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                    className: cn((child.props as Record<string, any>).className, "z-50 shadow-lg")
                                });
                            }
                            return null;
                        })}
